...
namespace Xcb.XFixes
{
	...
	public class CursorNotifyEvent : GenericEvent {
	...
	public struct Barrier : uint32 {
		...
		public VoidCookie create_pointer (Xcb.Connection connection, Drawable drawable, uint16 x1, uint16 y1, uint16 x2, uint16 y2, DirectionMask directions, [CCode (array_length_pos = 7.8)]uint16[] devices);
		[CCode (cname = "xcb_xfixes_create_pointer_barrier_checked", instance_pos = 1.1)]
		public VoidCookie create_pointer_checked (Xcb.Connection connection, Drawable drawable, uint16 x1, uint16 y1, uint16 x2, uint16 y2, DirectionMask directions, [CCode (array_length_pos = 7.8)]uint16[] devices);
	...
	[Flags, CCode (cname = "xcb_xfixes_direction_mask_t", cprefix =  "XCB_XFIXES_DIRECTION_MASK_", has_type_id = false)]
