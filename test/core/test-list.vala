...
public class Maia.TestList : Maia.TestCase
{
    ...
    private Core.List<int> m_ListNoSorted;
    private Core.List<int> m_List;
    ...
    set_up ()
    {
        m_ListNoSorted = new Core.List<int> ();
        m_List = new Core.List<int>.sorted ();
        m_List.compare_func = (Core.CompareFunc<int>)Maia.Core.direct_compare;
    ...
    test_list_erase ()
    {
        ...
        for (int cpt = 0; cpt < NB_KEYS; ++cpt)
        {
            ...
            Core.Iterator<int> found = m_List[m_Keys[index]];
