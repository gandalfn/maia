...
public class Maia.Cassowary.LinearExpression : Object
{
    // properties
    private Double m_Constant;
    private Map<AbstractVariable, Double> m_Terms;
    ...
    public Map<AbstractVariable, Double> terms {
        get {
            return m_Terms;
        }
    }
    ...
    public LinearExpression (AbstractVariable? inVariable, double inValue = 1.0, double inConstant = 0.0)
    {
        ...
        m_Terms = new Map<AbstractVariable, Double> ();
    ...
    protected LinearExpression.cloned (Double inConstant, Map<AbstractVariable, Double> inTerms)
    {
        m_Constant = (Double) inConstant.clone();
        m_Terms = new Map<AbstractVariable, Double> ();

        foreach (unowned Pair<AbstractVariable, Double> pair in inTerms)
        {
    ...
    multiply_me (double inX)
    {
        ...
        foreach (unowned Pair<AbstractVariable, Double> pair in m_Terms)
        {
    ...
    add_expression (LinearExpression inExpr, double inN = 1.0, AbstractVariable? inSubject = null, Tableau? inSolver = null)
    {
        ...
        foreach (unowned Pair<AbstractVariable, Double> pair in inExpr.m_Terms)
        {
