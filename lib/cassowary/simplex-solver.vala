...
public class Maia.Cassowary.SimplexSolver : Tableau
{
     ...
     * values).
     */
    private Array<AbstractVariable> m_StayMinusErrorVars;
     ...
     * values).
     */
    private Array<AbstractVariable> m_StayPlusErrorVars;

    /**
     * Give error variables for a non-required constraints,
     * maps to SlackVariable.
     *
     * Map Constraint to Set (of Variable).
     */
    private Map<Constraint, Set<AbstractVariable>> m_ErrorVars;

    /**
     * Return a lookup table giving the marker variable for
     * each constraints (used when deleting a constraint).
     *
     * Map Constraint to Variable.
     */
    private Map<Constraint, AbstractVariable> m_MarkerVars;

    private ObjectiveVariable m_Objective;

    /**
     * Map edit variables to EditInfo.
     *
    ...
    public Map<Constraint, AbstractVariable> constraint_map {
        get {
            return m_MarkerVars;
        }
    }
    ...
    {
        m_StayMinusErrorVars = new Array<AbstractVariable> ();
        m_StayPlusErrorVars = new Array<AbstractVariable> ();
        m_ErrorVars = new Map<Constraint, Set<AbstractVariable>> ();
        m_MarkerVars = new Map<Constraint, AbstractVariable> ();

        m_ResolvePair = new Pair<Double, Double> (new Double (0), new Double (0));

        m_Objective = new ObjectiveVariable.with_name ("Z");

        m_EditVarMap = new Map<Variable, EditInfo> ();
    ...
    choose_subject (LinearExpression inExpr) throws Error
    {
        ...
        unowned Map<AbstractVariable, Double> terms = inExpr.terms;

        foreach (unowned Pair<AbstractVariable, Double> pair in terms)
        {
            ...
            else
            {
                ...
                if (pair.first.is_restricted)
                {
                    ...
                    {
                        unowned Set<AbstractVariable> col = m_Columns[pair.first];
    ...
    new_expression (Constraint inConstraint, Pair<SlackVariable, SlackVariable> inEPlusEMinus, Double inPrevEConstant)
    {
        ...
        unowned Map<AbstractVariable, Double> cnTerms = cnExpr.terms;

        foreach (unowned Pair<AbstractVariable, Double> pair in cnTerms)
        {
            double c = pair.second.@value;
    ...
    optimize (ObjectiveVariable inVariable) throws Error
    {
        ...
        while (true)
        {
            ...
            unowned Map<AbstractVariable, Double> terms = zRow.terms;
            foreach (unowned Pair<AbstractVariable, Double> pair in terms)
            {
                double c = pair.second.@value;
            ...
            unowned Set<AbstractVariable> columnVars = m_Columns[entryVar];
            double r = 0.0;
    ...
    delta_edit_constant (double inDelta, AbstractVariable inPlusErrorVar, AbstractVariable inMinusErrorVar)
    {
        ...
        unowned Set<AbstractVariable> columnVars = m_Columns[inMinusErrorVar];
    ...
    dual_optimize () throws Error
    {
        ...
        {
            Iterator<AbstractVariable> iter = m_InfeasibleRows.iterator ();
            iter.next ();
            ...
            if (expr != null)
            {
                if (expr.constant < 0.0)
                {
                    ...
                    double r;
                    unowned Map<AbstractVariable, Double> terms = expr.terms;
                    foreach (unowned Pair<AbstractVariable, Double> pair in terms)
                    {
                        double c = pair.second.@value;
    ...
    {
        unowned Set<AbstractVariable>? cnset = m_ErrorVars[inConstraint];
        if (cnset == null)
        {
            Set<AbstractVariable> newSet = new Set<AbstractVariable> ();
     ...
     */
    public SimplexSolver
    add_lower_bound (AbstractVariable inVariable, double inLower) throws Error
    {
        LinearInequality cn = new LinearInequality.with_variable_expression (inVariable, Operator.GEQ, new LinearExpression.from_constant (inLower));
     ...
     */
    public SimplexSolver
    add_upper_bound (AbstractVariable inVariable, double inUpper) throws Error
    {
        LinearInequality cn = new LinearInequality.with_variable_expression (inVariable, Operator.LEQ, new LinearExpression.from_constant (inUpper));
        return add_constraint (cn);
    }
     ...
     */
    public SimplexSolver
    add_bounds (AbstractVariable inVariable, double inLower, double inUpper) throws Error
    {
        add_lower_bound (inVariable, inLower);
        add_upper_bound (inVariable, inUpper);
        return this;
    }
    ...
    {
        Pair<SlackVariable, SlackVariable> eplus_eminus = new Pair<SlackVariable, SlackVariable>.empty ();
    ...
    {
        Map<Variable, EditInfo> editVarMapCopy = new Map<Variable, EditInfo> ();

        foreach (unowned Pair<Variable, EditInfo> pair in m_EditVarMap)
        {
     ...
     *
     * @param inListOfPoints List of points to add weak stay constraints for.
     */
    public SimplexSolver add_point_stays (Array<Point> inListOfPoints) throws Error
    {
        double weight = 1.0;
        const double multiplier = 2.0;

        foreach (unowned Point point in inListOfPoints)
        {
            add_point_stay (point, weight);
            weight *= multiplier;
        }
    ...
    public SimplexSolver
    add_point_stay (Point inPoint, double inWeight = 1.0) throws Error
    {
        add_stay (inPoint.x, Strength.weak, inWeight);
        add_stay (inPoint.y, Strength.weak, inWeight);
    ...
    remove_constraint (Constraint inConstraint) throws Error
    {
        ...
        unowned Set<AbstractVariable> eVars = m_ErrorVars[inConstraint];
        ...
        if (row_expression (marker) == null)
        {
            ...
            unowned Set<AbstractVariable> col = m_Columns[marker];
            ...
            if (exitVar == null)
            {
                ...
                else
                {
                    ...
                    Iterator<AbstractVariable> iter = col.iterator ();
                    iter.next ();
                    exitVar = iter.get ();
                }
            }
     ...
     */
    public void
    resolve_pair (Pair<Double, Double> m_NewEditConstants) throws Error
    {
        foreach (unowned Pair<Variable, EditInfo> pair in m_EditVarMap)
        {
